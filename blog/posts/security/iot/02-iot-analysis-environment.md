---
title: "iot-env-config"
description: "个人 iot 分析所用的环境"
date: "2024-01-12T07:43:00.000Z"
updated: "2024-08-20T02:30:17.000Z"
tags: []
draft: false
layout: "post"
...
workdir=/opt/workspace/qemu-imgs
...
8.   安装完成后，到 `fat.config` 中修改默认内容，参考：
     ...
     [DEFAULT]
     sudo_password=root
     firmadyne_path=/opt/workspace/fat/firmadyne
     ```

### usage

安装完成后，直接通过 `/opt/workspace/fat/fat.py firmware.bin` 来解析固件

## frida

get src here：[frida/frida](https://github.com/frida/frida.git)；Android、Linux、Windows 等平台上的动态插桩工具，拦截应用程序的函数调用、监视数据传输以及修改应用程序的行为，从而进行各种各样的分析和测试操作；`pip install frida-tools`，安装完成后将其添加到环境变量：`export PATH=$PATH:~/.local/bin`，也可以写到 `~/.bashrc` 中
