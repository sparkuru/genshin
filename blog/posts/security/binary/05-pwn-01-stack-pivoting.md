---
title: "ctf | 01_pwn-stack-pivoting"
description: "格式化字符串漏洞、栈迁移、ret2libc"
date: "2022-11-17T16:57:00.000Z"
updated: "2024-04-27T02:39:10.000Z"
tags: ["CTF","pwn"]
draft: false
layout: "post"
slug: "pwn-01-stack-pivoting"
---
...
![image-20221113200217201](https://tataramoriko-oss.oss-cn-shenzhen.aliyuncs.com/markdown/image-20221113200217201.png)
...
![image-20221113154554924](https://tataramoriko-oss.oss-cn-shenzhen.aliyuncs.com/markdown/image-20221113154554924.png)

题目很直球，执行完是这样的

1. **leak_stack** 选项，

	![image-20221113154658214](https://tataramoriko-oss.oss-cn-shenzhen.aliyuncs.com/markdown/image-20221113154658214.png)

  很直接的格式化字符串漏洞

2. **leak_libc** 选项，

	![image-20221113154724044](https://tataramoriko-oss.oss-cn-shenzhen.aliyuncs.com/markdown/image-20221113154724044.png)

  **buf** 大小为 **(0x70 - 0x8) Bytes**，可以往其中输入 **0x78 Bytes** 的内容，这里很明显是仅可以覆盖到 **ret**

3. **rop** 选项，

	![image-20221113154824117](https://tataramoriko-oss.oss-cn-shenzhen.aliyuncs.com/markdown/image-20221113154824117.png)
...
![image-20221113155718460](https://tataramoriko-oss.oss-cn-shenzhen.aliyuncs.com/markdown/image-20221113155718460.png)
...
2. 还有另一种逃课方法，这个需要 **pwndbg** 调试，在对应的 **printf** 函数被调用处下断点，然后使用指令 `fmtarg <想要泄露内容的地址>`，如图所示
	...
	![](https://tataramoriko-oss.oss-cn-shenzhen.aliyuncs.com/markdown/image-20221113160139206.png)
...
![image-20221113202816897](https://tataramoriko-oss.oss-cn-shenzhen.aliyuncs.com/markdown/image-20221113202816897.png)
...
![image-20221114205357670](https://tataramoriko-oss.oss-cn-shenzhen.aliyuncs.com/markdown/image-20221114205357670.png)
...
![image-20221114205716142](https://tataramoriko-oss.oss-cn-shenzhen.aliyuncs.com/markdown/image-20221114205716142.png)
...
1. 观察函数在调用和返回时的栈的变化（栈向低地址生长）
	...
	![call 函数调用时栈的变化](https://tataramoriko-oss.oss-cn-shenzhen.aliyuncs.com/markdown/image-20221117181342306.png)

	关注 **rsp（栈顶）**、**rbp（栈底）** 中内容的变化，这两个寄存器的值规划着当前工作栈的区间

	![ret 时栈的变化](https://tataramoriko-oss.oss-cn-shenzhen.aliyuncs.com/markdown/image-20221117181434423.png)
...
2. 仔细观察两张图中间部分的状态可以得知，栈实际上是用 **rbp** 的值来确定栈底位置的。如果人为的修改了 **保存在栈上的 rbp** 的值（图中 **0x1028** 的位置中的 **值**）则可以控制栈的区间位置到不同的地方。例如，将位置 **0x1028** 中的值 **调用者的 rbp 地址** 修改为 **0x1060**，则返回后的栈的变化被修改如下
	...
	![人为修改](https://tataramoriko-oss.oss-cn-shenzhen.aliyuncs.com/markdown/image-20221117182649835.png)
	...
	![image-20221117183547744](https://tataramoriko-oss.oss-cn-shenzhen.aliyuncs.com/markdown/image-20221117183547744.png)

	本题中栈上内容的布置如下

	![image-20221117193042344](https://tataramoriko-oss.oss-cn-shenzhen.aliyuncs.com/markdown/image-20221117193042344.png)
	...
	![image-20221117213433277](https://tataramoriko-oss.oss-cn-shenzhen.aliyuncs.com/markdown/image-20221117213433277.png)
