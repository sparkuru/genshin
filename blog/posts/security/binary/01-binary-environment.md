---
title: "binary-env-config"
description: "env config for binary selfuse"
date: "2024-03-12T12:29:00.000Z"
updated: "2024-08-20T02:48:53.000Z"
tags: []
draft: false
layout: "post"
slug: "binary-environment"
---
...
1.   创建环境根目录：`sudo mkdir -p /opt/workspace/pwnenv/pwndbg`，`sudo chown -R wkyuu:wkyuu /home/app`
2.   `git clone https://github.com/pwndbg/pwndbg.git /opt/workspace/pwnenv/pwndbg/repo`
3.   `cd /opt/workspace/pwnenv/pwndbg/repo`，`chmod +x ./setup.sh`，`sudo ./setup.sh` 执行安装
...
1.   ~~pwngdb 包含 angelheap，通过 curl 直接获取，确保 pwndbg 的根路径是 `/opt/workspace/pwnenv/pwndbg`~~

     1.   ~~`curl -o /opt/workspace/pwnenv/pwndbg/repo/pwndbg/pwngdb.py https://raw.githubusercontent.com/scwuaptx/Pwngdb/master/pwndbg/pwngdb.py`、`curl -o /opt/workspace/pwnenv/pwndbg/repo/pwndbg/commands/pwngdb.py https://raw.githubusercontent.com/scwuaptx/Pwngdb/master/pwndbg/commands/pwngdb.py`~~
     2.   ~~`curl -o /opt/workspace/pwnenv/pwndbg/repo/pwndbg/angelheap.py https://raw.githubusercontent.com/scwuaptx/Pwngdb/master/pwndbg/angelheap.py`、`curl -o /opt/workspace/pwnenv/pwndbg/repo/pwndbg/commands/angelheap.py https://raw.githubusercontent.com/scwuaptx/Pwngdb/master/pwndbg/commands/angelheap.py`~~

2.   splitmind 的主要作用是在使用 tmux 时方便将各种 pwndbg 输出重定向到不同的区块

     1.   获取插件内容：`git clone https://github.com/jerdna-regeiz/splitmind.git /opt/workspace/pwnenv/pwndbg/plugins/splitmind`

     2.   `echo "source /opt/workspace/pwnenv/pwndbg/plugins/splitmind/gdbinit.py" >> ~/.gdbinit`
...
```ini
source /opt/workspace/pwnenv/pwndbg/repo/gdbinit.py
source /opt/workspace/pwnenv/pwndbg/plugins/splitmind/gdbinit.py
...
1.   `git clone https://github.com/matrix1001/glibc-all-in-one.git /opt/workspace/pwnenv/glibc_all_in_one`
2.   `cd /opt/workspace/pwnenv/glibc_all_in_one`，`chmod +x update_list download extract`，更新列表：`./update_list`，查看更新的结果：`cat list`，会列出一大串不同版本的 `2.xx-ubuntux.x`，根据 elf 文件所需的 libc 信息获取之，这里以 `2.27-3ubuntu1_amd64` 为例
3.   下载之：`./download 2.27-3ubuntu1_amd64`，下载完成的文件在 `/opt/workspace/pwnenv/glibc_all_in_one/libs/2.27-3ubuntu1_amd64`  里，大部分环境所需的只有 `2.27-3ubuntu1_amd64/libc-2.27.so` 和 `2.27-3ubuntu1_amd64/ld-2.27.so`，分别对应 `libc.so.6 => /lib/x86_64-linux-gnu/libc.so.6` 和 `/lib64/ld-linux-x86-64.so.2`
