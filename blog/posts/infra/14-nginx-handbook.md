---
title: "nginx"
description: "Nginx 相关配置。"
date: "2022-03-18T04:39:00.000Z"
updated: "2024-12-24T05:50:16.000Z"
tags: []
draft: false
layout: "post"
slug: "nginx-handbook"
...
7.   编写 stream 模块允许代理 tcp 流量，在 `nginx.conf` 中配置以下内容**（这是一个 相对无效 的配置）**：
     ...
     2.   该项的逻辑是，当一个 request 来到 80 端口时，nginx 智能地根据 SNI（Server Name Indication）来分流这些 request，例如看到是 `CONNECT HTTP blog.majo.im` 就访问 `/srv/www/typecho/index.php`；看到是 `CONNECT TCP mc.majo.im`，则被 stream 里的 `$backend` 给 proxy 到 127.0.0.1:5505 实现 tcp 的转发
...
2.   **使用 stream 实现对 tcp 流量的加密**：假设自己开发的系统使用纯 tcp 通信（或者私有通信协议），但是又需要加密，则 stream 就可以帮助实现之；例如 client 和 server 之间的 tcp 连接要被加密，server 在服务器上开放在 5555 端口，对外的端口是 5556，则可以使用 nginx stream 将 client 与 5556 之间的通信使用 ssl/tls 来加密，然后再让 nginx 透明地将数据从 5556 传输给 5555 （client -> 5556 -> nginx_stream -> 5555 -> server）
...
经过多个应用的测试，这种操作实际上已经破产了。可以举一个例子简单来说明，假设我要在本地通过 mysql 连接到 mysql server（虽然 mysql 本身就支持自己的 ssl 操作，但是其他纯 tcp、udp 的应用不一定支持，这里就把 mysql 假设成不支持自己的 ssl 来举例），mysql 本身没有指定 ssl 连接的情况下，就不会主动使用 ssl 套件来与 nginx stream 来进行 ssl handshake，自然无法被 nginx stream 承认为合法的连接，也就无法建立通信；但是假如 mysql 指定了使用 mysql 连接，虽然可以和 nginx stream 连接了，但是当一个带有 "请求 ssl 连接" 的 mysql request 被 nginx stream 转发给内网的 3306 时，内网的 3306 有没有开启 ssl 是一方面，更重要的是内网假如返回一个 ssl handshake 请求与本地的 mysql 连接，此时双方信息不同步，也无法建立连接。
...
3.   自动更新证书服务：[acmesh-official/acme.sh](https://github.com/acmesh-official/acme.sh.git)，这里使用 dns 方式生成证书（下文内容客制化更改 domain 信息）
     ...
     4.   正式签发，有两种方式
          ...
          1.   **（完全自动，十分推荐）**创建一个脚本文件：`touch /opt/workspace/acme/scripts.sh`，填入以下内容（以下脚本需要按照具体情况来弄，但是如果你是按照我前文的配置，则基本只需要修改 `wkyuu` 为自己的用户名，以及 `majo.im` 修改成自己的域名）：
...
下面是一个用于测试 tcp、udp、http、https 的简单脚本
...
def test_udp_connection(host, port):
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
            sock.settimeout(10)
            sock.sendto(b"test", (host, port))	# UDP是无连接的，只尝试发送数据
            sock.recvfrom(1024)	# 尝试接收响应
            print(f"UDP connection to {host}:{port} succeeded.")
    except Exception as e:
        print(f"UDP connection to {host}:{port} failed: {e}")

def test_http_connection(url):
    try:
        response = requests.get(url, timeout=10)
        print(f"HTTP connection to {url} succeeded, status code: {response.status_code}")
    except Exception as e:
...
def test_https_connection(url):
    try:
        ...
        print(f"HTTPS connection to {url} succeeded, status code: {response.status_code}")
    except Exception as e:
        print(f"HTTPS connection to {url} failed: {e}")

host = 'www.majo.im'
port = 9441
test_tcp_connection(host, port)
test_udp_connection(host, port)
test_http_connection(f"http://{host}")
test_https_connection(f"https://{host}")
```

## references

1.   [使用 nginx 配置二级域名](https://blog.csdn.net/a_helloword/article/details/84316291)
2.   [nginx 配置禁止访问目录或禁止访问目录下的文件](https://segmentfault.com/a/1190000040649628)
3.   [Linux 运维 | nginx 访问控制与参数调优](https://segmentfault.com/a/1190000018505993)
4.   [使用 openssl 创建 https 证书](https://cloud.tencent.com/developer/article/1548350)
5.   [在 Nginx 中配置二级域名](https://mincong.io/cn/nginx-subdomains/)
6.   [NGINX Stream 模块原理及代码分析](https://www.nginx.org.cn/article/detail/98)
7.   [Building nginx from Sources](https://nginx.org/en/docs/configure.html)
8.   [玩转 Nginx 之：使用 Lua 扩展 Nginx 功能](https://cloud.tencent.com/developer/article/1043931)
9.   [Centos7 安装 postfix 和 postfix 配置](https://www.31idc.com/helpcontent/274.html)
10.   [Linux中Postfix邮件发送配置（三）](https://www.cnblogs.com/flytor/p/11447686.html)
11.   [Postfix Issue: Connection Timed Out](https://unix.stackexchange.com/questions/615825/postfix-issue-connection-timed-out)
12.   [阿里云服务器 centos7 中继邮箱+转发服务 详细配置](https://www.cnblogs.com/telegram/p/10718477.html)
13.   [阿里云、腾讯云封锁25端口发送不了邮件该如何解决](https://www.relaypod.com/jishuziliao/20201223/1055.html)
14.   [linux搭建邮件服务器 - postifx + SSL + 465端口配置](https://www.cnblogs.com/wangbingbing/p/15419974.html)
15.
