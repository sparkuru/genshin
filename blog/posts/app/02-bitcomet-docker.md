---
title: "bitcomet-in-docker-for-linux"
description: "Linux 下使用 docker 搭建 bitcomet 应用"
date: "2024-04-14T09:21:00.000Z"
updated: "2024-08-20T02:47:36.000Z"
tags: []
draft: false
layout: "post"
...
又名 docker 配置文件修改参考。bitcomet for Linux 安装 [参考](http://wiki-zh.bitcomet.com/linux版bitcomet安装指南)，在 docker 化需求下，我使用 docker-compose 将其启动，下面是我的配置文件参考：
...
services:
  bitcomet:
    ...
    volumes:
      ...
      - /opt/workspace/bitcomet/appdata:/container-data/.config/BitComet:rw
      # mounts a host directory into the container to store downloaded files
      - /opt/workspace/bitcomet/downloads:/container-data/Downloads:rw
    ports:
...
# sudo docker-compose -f /opt/workspace/bitcomet/bitcomet.yml up -d
```
...
1.   首先正常启动该 docker 镜像：`sudo docker-compose -f /opt/workspace/bitcomet/bitcomet.yml up`
...
3.   查看所有运行的进程 `ps -aux`，输出如下：
     ...
     sandbox       67  0.0  0.4  66400 18260 ?        S    08:49   0:00 /usr/bin/openbox --config-file /container-data/.config/openbox/lxqt-rc.xml
     sandbox       70 23.9  5.1 104551672 206556 ?    Sl   08:49   1:27 /container-data/BitCometApp/usr/bin/BitComet
...
5.   观测 `cat /etc/supervisor/supervisord.conf` 文件可以得知，其引入了 `/etc/supervisor/conf.d/*.conf` 下的文件，进一步获取 `cat /etc/supervisor/conf.d` 得到以下配置文件，现在将其下载到主机里：`docker cp docker_id:/etc/supervisor /opt/workspace/bitcomet/supervisor`，后续可以直接在外部修改这个文件来控制 docker 内部关键参数。**之所以先启动后再获取内部配置文件，是因为防止后续再次启动时如果挂载了一个空文件（夹）会导致内部文件丢失**

6.   参考 x11vnc 的 [配置](https://linux.die.net/man/1/x11vnc)，需要在原配置的情况下新增 `-autoport` 来指定新的 port，最终完整配置文件 `/opt/workspace/bitcomet/supervisor/conf.d/supervisord.conf` 如下：
     ...
     services:
       bitcomet:
         ...
         volumes:
           - /opt/workspace/bitcomet/appdata:/container-data/.config/BitComet:rw
           - /opt/workspace/bitcomet/downloads:/container-data/Downloads:rw
           - /opt/workspace/bitcomet/supervisor:/etc/supervisor
         ports:
