...
if [ -d "$workdir/himitsu" ]; then
    src_file_path="$workdir/himitsu/Thanox-Backup.zip"
else
    echo "${red}Thanox-Backup.zip not found${nc}"
    exit 1
fi

target_file_path="$workdir/thanox-backup.zip"
