...
Before patching agent-specific Trellis files such as `.agents/**`, `.claude/**`, `.codex/**`, `.cursor/**`, `.opencode/**`, `.gemini/**`, `opencode.json`, or `AGENTS.md`, check whether the target is tracked or ignored with `git ls-files -- <path>` and `git check-ignore -v -- <path>`.
