...
def color(text: str, color_code: int = 0) -> str:
    ...
    return color_table[color_code].format(
        clean_path(text) if os.path.sep in text else text
    )
...
class PathManager:
    ...
    def load_paths(self) -> List[str]:
        ...
        try:
            ...
            print(
                color(f"Config file corrupted: {self.config_file}", CLI_COLORS["ERROR"])
            )
    ...
    def add_path(self, path: str, path_color: int) -> None:
        ...
        if path in self.paths:
            print(
                f"{color('|', CLI_COLORS['TITLE'])} Path [{color(path, path_color)}] already exists."
            )
        else:
            self.paths.append(path)
            self.save_paths()
            print(
                f"{color('|', CLI_COLORS['TITLE'])} Path [{color(path, path_color)}] added."
            )
    ...
    def delete_path(self, path: str, path_color: int, num: int = 0) -> None:
        ...
        if num != 0:
            if num > len(self.paths) or num <= 0:
                print(
                    f"{color('|', CLI_COLORS['TITLE'])} Path number [{color(str(num), CLI_COLORS['NUMBER'])}] out of range."
                )
                return
        ...
        if path in self.paths:
            ...
            self.save_paths()
            print(
                f"{color('|', CLI_COLORS['TITLE'])} Path [{color(path, path_color)}] deleted."
            )
        else:
            print(
                f"{color('|', CLI_COLORS['TITLE'])} Path [{color(path, path_color)}] doesn't exist."
            )
    ...
    def list_paths(self, path_color: int, num: int = 0) -> None:
        ...
        for i, path in enumerate(self.paths, 1):
            ...
            status_color = (
                ...
                else CLI_COLORS["ERROR"]
            )
            print(
                f"{color('|', CLI_COLORS['TITLE'])} [{color(str(i), CLI_COLORS['NUMBER'])}] {color(exists, status_color)} {color(abs_path, path_color)}"
            )
    ...
    def cd_to_path(self, num: int, path_color: int) -> None:
        ...
        if not os.path.exists(path):
            print(
                f"{color('|', CLI_COLORS['TITLE'])} Warning: Path [{color(path, path_color)}] doesn't exist.",
                file=sys.stderr,
            )
    ...
    def show_config(self, path_color: int) -> None:
        ...
        print(
            f"{color('Config file:', CLI_COLORS['TITLE'])} {color(self.config_file, path_color)}"
        )
...
def main():
    ...
    parser.add_argument(
        "-n", "--num", type=int, default=0, help="Specify path number to operate on"
    )
    parser.add_argument(
        "-l", "--list", action="store_true", help="List all stored paths"
    )
    parser.add_argument(
        "-g", "--goto", type=int, help="Output path by number for use with cd command"
    )
    parser.add_argument(
        "-c", "--config", action="store_true", help="Show config file path"
    )
    parser.add_argument(
        "-p", "--plain", action="store_true", help="Don't show colored output"
    )
