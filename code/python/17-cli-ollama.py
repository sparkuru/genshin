...
class ColoredArgumentParser(argparse.ArgumentParser):
    """Enhanced argument parser with colored help output"""
...
def create_example_text(script_name: str, examples: list, notes: list = None) -> str:
    """Generate formatted help text with examples and notes"""
...
DEFAULT_CONFIG_PATH = os.path.join(OLLAMA_DIR_PATH, CONFIG_FILENAME)
...
def show_loading_animation():
    """Display animated loading spinner with elapsed time"""
    animation = ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"]
    i = 0
    ...
    while is_generating:
        ...
        sys.stdout.write(
            CLIStyle.color(
                f"\r{animation[i]} Generating response... ({elapsed:.1f}s)", 6
            )
        )
        sys.stdout.flush()
        time.sleep(0.1)
...
class OllamaClient:
    def __init__(self, ip, port, ssl=False):
        ...
        try:
            ...
            print(
                CLIStyle.color(f"Failed to connect to Ollama service at {self.host}", 2)
            )
    ...
    def _get_models(self):
        """Fetch raw model list data from Ollama API"""
        try:
    ...
    def show_model_list(self):
        """Display available models in simple list format"""
        ...
        for model in models:
            ...
            print(
                CLIStyle.color(
                    f"- {model['model']} ({param_size} params, {formatted_size})", 7
                )
            )

    def show_model_selection(self, current_model=None):
        """Display interactive model selection interface"""
        try:
            models = self._get_models()
            if not models:
                print(CLIStyle.color("\nNo models available.", 2))
            ...
            for idx, model in enumerate(models, 1):
                ...
                table.add_row(str(idx), model["model"], param_size, formatted_size)
            ...
            console.print(
                Panel(
                    "[cyan]Enter number to select (1-{}) or 0 to quit[/cyan]".format(
                        len(models)
                    ),
                    border_style="cyan",
                    box=box.ROUNDED,
            ...
            try:
                choice = IntPrompt.ask("\nSelect model", console=console, default=1)
    ...
    def generate_config_file(self, config_path):
        ...
        if os.path.exists(config_path):
            try:
                ...
                print(
                    CLIStyle.color(
                        f"Warning: Existing config file '{config_path}' is invalid, using default values",
                        4,
                    )
                )
        ...
        try:
            ...
            print(CLIStyle.color(f"Config file created at: {config_path}", 3))
            print(CLIStyle.color("\nConfig contents:", 7))
...
def check_required_params(args, config):
    ...
    if not args["config"]:
        if not args["ip"] and not config.get("ip"):
            ...
            suggestions.append(
                "Use -i/--ip to specify server IP or use a config file (-c)"
            )
        if not args["port"] and not config.get("port"):
            errors.append("No server port specified")
            suggestions.append(
                "Use -p/--port to specify server port or use a config file (-c)"
            )
    ...
    if errors:
        ...
        print(
            CLIStyle.color(
                f"   {os.path.basename(sys.argv[0])} --new-config {default_config}", 8
            )
        )
        ...
        print(
            CLIStyle.color(
                f"   {os.path.basename(sys.argv[0])} -i 127.0.0.1 -p 11434 -m llama3.2:1b",
                8,
            )
        )
        sys.exit(1)
...
def get_user_input(prompt_text=None):
    """Enhanced input handler with cursor movement and backspace support"""
    buffer = []
    cursor_pos = 0

    def refresh_line():
        """Update terminal display with current buffer content"""
        print("\r" + " " * 100 + "\r", end="")
...
def show_current_config(args, config):
    ...
    current_config = {
        ...
        if explicitly_set["pre_prompt"]
        else config.get("pre_prompt", ""),
    }
    ...
    if args["config"]:
        ...
        table.add_row("Config File", DEFAULT_CONFIG_PATH, "Default")
...
def get_user_confirmation(prompt_text):
    """Get user confirmation (y/n) input"""
...
def update_config(args, config_path):
    """Update configuration file with current parameters"""
    if not os.path.exists(config_path):
        ...
        try:
            ...
            if not get_user_confirmation(
                f"Invalid config file at: {config_path}\nOverwrite with new config?"
            ):
                return False
            config = {}
...
def main():
    ...
    if os.path.exists(DEFAULT_CONFIG_PATH):
        try:
            ...
            print(
                CLIStyle.color(
                    f"\nWarning: Default config file '{DEFAULT_CONFIG_PATH}' is invalid",
                    4,
                )
            )
    ...
    ap.add_argument(
        "-i", "--ip", type=str, help="API host address (default: 127.0.0.1)"
    )
    ...
    ap.add_argument(
        "--update",
        action="store_true",
        help="Update config file with current parameters",
    )
    ...
    if args["config"]:
        ...
        if not config:
            ...
            if os.path.exists(DEFAULT_CONFIG_PATH):
                ...
                print(
                    CLIStyle.color(f"  {script_name} --new-config {args['config']}", 8)
                )
                print(CLIStyle.color("2. Create default config file:", 7))
                print(
                    CLIStyle.color(
                        f"  {script_name} --new-config {DEFAULT_CONFIG_PATH}", 8
                    )
                )
            return
    else:
    ...
    if args["exit"]:
        try:
            messages = []
            if args["pre_prompt"]:
                messages.append({"role": "system", "content": args["pre_prompt"]})
            messages.append({"role": "user", "content": args["exit"]})
            ...
            try:
                ...
                print(CLIStyle.color(SEPARATOR_CHAR * SEPARATOR_LENGTH, 8))
            except Exception:
                sys.exit(1)
            return
        except Exception:
    ...
    print(
        CLIStyle.color("Starting chat with model: ", 3)
        + CLIStyle.color(args["model"], 6)
    )
