...
def encrypt_file(input_path: str, output_path: str, password: str, salt: bytes):
    ...
    cipher = Cipher(
        algorithms.AES(key), modes.CBC(initialization_vector), backend=default_backend()
    )
...
def main():
    ...
    required_group.add_argument(
        "-i", "--input", required=True, help="Specify input file or directory path"
    )

    optional_group = parser.add_argument_group(f"{color('Optional arguments', 2)}")
    optional_group.add_argument(
        "-o", "--output", help="Specify output file or directory path"
    )
    optional_group.add_argument(
